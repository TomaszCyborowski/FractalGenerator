...
{
    public sealed class MagnetFractal2 : AbstractFractal
    {
        private readonly FractalV1ParametersControl parametersControl;        
                
        public override string FractalDisplayName { get => "Magnet Fractal 2"; }
        public override bool SupportsZoom { get => true; }

        public MagnetFractal2()
        {
        ...
        protected override void CalculatePixelValue(double a, double b, int pixelXposition, int pixelYposition)
        {
            ...
            {                
                Z = Complex.Pow((Complex.Pow(Z, new Complex(2, 0)) + (C - 1)) / (2 * Z + (C - 2)), new Complex(2, 0));
